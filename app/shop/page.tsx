...
export default function ShopPage() {
  ...
  return <main className="shop-page">
    ...
    <ShopCatalog />
    <section className="package-section" id="packages"><div className="shell"><div className="catalogue-heading"><div><p className="section-kicker">Start with an outcome</p><h2>Smart packages</h2></div><p>Packages are design starting points. Exact quantities and models are confirmed after a compatibility review.</p></div><div className="package-grid">{packages.map((item, index) => <article key={item[0]}><span>0{index + 1}</span><small>{item[2]}</small><h3>{item[0]}</h3><p>{item[1]}</p><a href={`mailto:info@ki-analysix.com?subject=${encodeURIComponent(`Package enquiry: ${item[0]}`)}`}>Configure this package <b>↗</b></a></article>)}</div></div></section>
    <SystemBuilder />
