...
export const metadata = { title: "Smart Technology Shop | KI Analysix System", description: "Complete TAOS smart-switch, smart-lighting and control-panel ranges with Tuya-compatible products, installation and project configuration." };
...
export default function ShopPage() {
  ...
  return <main className="shop-page">
    ...
    <section className="shop-hero"><div className="shop-hero-grid" aria-hidden="true" /><div className="shell shop-hero-layout"><div><p className="eyebrow"><span /> Curated smart technology</p><h1>Choose devices that work <em>together.</em></h1><p>Browse complete smart-switch, smart-lighting and control-panel families from the TAOS catalogue, with compatibility, wiring and installation guidance built into every buying path.</p><div className="hero-actions"><a className="button button-primary" href="#catalogue">Browse products <span>↓</span></a><a className="button button-ghost" href="#builder">Build your system <span>↗</span></a></div></div><aside><div><span>Catalogue range</span><strong>{products.length}</strong><small>represented products</small></div><div><span>Product families</span><strong>{categoryCount}</strong><small>from one connected ecosystem</small></div><p>Supplier stock, certification and final Tuya/Smart Life pairing are verified before an order is accepted.</p></aside></div></section>
