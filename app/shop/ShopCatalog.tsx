...
export default function ShopCatalog() {
  ...
  return <>
    ...
    <section className="catalogue shell" id="catalogue">
      <div className="catalogue-heading"><div><p className="section-kicker light">Curated launch range</p><h2>{filtered.length} products</h2></div><p>Prices and live stock will be added after supplier verification. Product images are supplier catalogue references; the selected finish is confirmed with every request.</p></div>
      <div className="product-grid">{filtered.map((product) => {
        ...
        const imageName = product.model.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
        return <article className="product-card" key={product.sku}>
          ...
          <div className="product-media"><img src={`${base}/products/${imageName}.webp`} alt={`${product.name}, supplier catalogue image`} loading="lazy" /></div>
